...
public class AddContent extends Activity implements OnClickListener {
    ...
    private ImageView c_img;//图片
    private VideoView v_video;//视频
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dbWriter = notesDB.getWritableDatabase();//读取数据
        initView();
    }
//判断是文字还是图片还是视频
    public void initView() {
        ...
        if (val.equals("2")) {//图片
            ...
            phoneFile = new File(Environment.getExternalStorageDirectory()
                    .getAbsoluteFile() + "/" + getTime() + ".jpg");//以时间命名图片
            iimg.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(phoneFile));//储存照片
